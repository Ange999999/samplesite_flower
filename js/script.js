...
function setFadeElement() {
  ...
  //出現範囲の指定
  var contentsTop = Math.round($('#area-1').offset().top); //要素までの高さを取得
  var contentsH = $('#area-1').outerHeight(true); //要素の高さを取得
...
if (splash_text != myYear + myMonth + myDate) {
  ...
  setTimeout(function () {
    ...
    $('#splash_logo').fadeIn(1000, function () {
      setTimeout(function () {
        $('#splash_logo').fadeOUt(1000); //1秒かけてロゴがフェードアウト
      }, 1000); //１秒後に処理を実行
      setTimeouot(function () {
        ...
        $('#splash').fadeOUt(1000, function () {
          ...
          var myMonth = String(myD.getMonth() + 1);
          var myDate = String(myD.getDate());
          $.cookie('accessdate', myYear + myMonth + myDate); //access dateキーで年月日を記録
        });
      }, 1700); //1.7秒後に処理を実行
    });
  }, 1000); //1秒後に処理を実行
} else {
  $('#splash').css('display', 'none'); //同じ日付で2回目ならローディング画面非表示
}
